...
def process_image(input_image):
    ...
    # cv2.imread('CarND-Advanced-Lane-Lines/test_images/straight_lines1.jpg')
    ...
    if is_first_frame:
        ...
        ploty = np.linspace(0, binary_warped.shape[0]-1, binary_warped.shape[0]-1 )
        left_fitx = left_fit[0]*ploty**2 + left_fit[1]*ploty + left_fit[2]
        right_fitx = right_fit[0]*ploty**2 + right_fit[1]*ploty + right_fit[2]
        err_ind1 = np.where(left_fitx < 0)
        err_ind2 = np.where(right_fitx > binary_warped.shape[1])
        err_ind = np.append(err_ind1, err_ind2)
        if err_ind.size != 0:
            # it's is super unefficient however for code simplicity I'll do it
            ploty = np.delete(ploty, err_ind)
            right_fitx = np.delete(right_fitx, err_ind)
            left_fitx = np.delete(left_fitx, err_ind)
    ...
    # write_output_img(test_img, 'failed_here.jpg')
    # print((max(ploty)))
    # print(max(right_fitx))
    ...
    # write_output_img(color_warp, 'Filled_Area.jpg')
    ...
    cv2.putText(result,'right: '+str(right_curverad) + ', left: ' + str(left_curverad), (300,650), 0, 1,(0,0,255),2)
    return result

img =  cv2.imread('CarND-Advanced-Lane-Lines/test_images/test5.jpg')
ret = process_image(img)
write_output_img(ret, "result_image.jpg")
plt.imshow(ret)
plt.show()
